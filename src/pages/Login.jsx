...
function Login() {
    ...
    return (
        <div className="h-dvh flex justify-center items-center relative">
            ...
            <div
                className="w-full rounded-lg overflow-hidden relative outline-none outline-1 outline-offset-2 outline-neutral-50 max-w-[350px] md:max-w-[400px] custom-shadow">
                ...
                <div className="z-10 relative w-full h-[80%] flex flex-col gap-14 px-5 py-10">
                    <div className="flex flex-col items-center">
                        <h1 className="text-3xl text-center font-bold text-neutral-200">Login</h1>
                    ...
                    <form className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                onBlur={() => {
                                    setTouchedStatus({...touchedStatus, isPasswordTouched: true})
                                }}
                            ...
                            {!isPasswordValid && touchedStatus.isPasswordTouched &&
                                <span className="text-xs text-red-500">
                                    Password Invalid, must be 8 characters long and have 1 Uppercase, 1 Lowercase and 1 Symbol
                                </span>}
                        </div>
                        {/*when click go and try login*/}
                        <button onClick={handleSubmit}
                                className="px-3 py-2 bg-primary text-neutral-200 rounded outline-button">Iniciar
                            Sesion
                        </button>
                        <Link to={"/signup"}
                              className="text-primary hover:underline text-center text-sm outline-none focus:underline focus:text-blue-500">Aun
                            no tienes cuenta? crea una aca</Link>
                    </form>
                </div>
            </div>
        </div>
